 ...
 * Coleganet.
 *
 * Coleganet is an open source decentralised communications
 ...
 * websites, charitable organisations, etc.). Coleganet also provides DNS mobility
...
define ( 'PLATFORM_NAME',           'Coleganet' );
...
define ( 'DIRECTORY_REALM',            'COL_EGANET');
define ( 'DIRECTORY_FALLBACK_MASTER',  'https://inboxwall.com');

$DIRECTORY_FALLBACK_SERVERS = array( 
	'https://coleganet.es',
	'https://coleganet.co.il',
        'https://coleganet.asia'
);
...
define ( 'ITEM_DOC',             0x1000);   // Coleganet only, define here so that item import does the right thing
...
function login($register = false, $form_id = 'main-login', $hiddens=false) {
	...
	$reg = array(
		'title' => t('Create an account to access services and applications within the Coleganet'),
		'desc' => t('Register'),
...
/**
 * @brief Returns Coleganet's root directory.
 *
 * @return string
 */
function appdirpath() {
 ...
 *
 * If a Coleganet hub is available over https it must have a publicly valid
 * certificate.
 */
function cert_bad_email() {
	...
	$subject = email_header_encode(sprintf(t('[Coleganet] Website SSL error for %s'), App::get_hostname()));
	mail(App::$config['system']['admin_email'], $subject, $email_msg,
...
function check_cron_broken() {
	...
	$subject = email_header_encode(sprintf(t('[Coleganet] Cron tasks not running on %s'), App::get_hostname()));
	mail(App::$config['system']['admin_email'], $subject, $email_msg,
		'From: Administrator' . '@' . App::get_hostname() . "\n"
		. 'Content-type: text/plain; charset=UTF-8' . "\n"
		. 'Content-transfer-encoding: 8bit' );
