...
function flattrwidget_settings_post($a,$s) {
    ...
    if ($ftitle == '') {
	$ftitle = $c['channel_name'].' on The Coleganet';
    }
...
function flattrwidget_settings(&$a,&$s) {
	...
	$sc .= replace_macros(get_markup_template('field_input.tpl'), array(
		'$field'	=> array('flattrwidget-thingtitle', t('Title of the Thing to flattr'), $ftitle, t('If empty "channel name on The Coleganet" will be used'))
	));
