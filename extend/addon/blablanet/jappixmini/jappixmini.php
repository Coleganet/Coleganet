...
function jappixmini_settings(&$a, &$s) {
	...
	$sc .= '<strong>' . t('Status:') . '</strong> Addon knows ' . $address_cnt . ' Jabber addresses of ' . $contact_cnt . ' Coleganet contacts (takes some time, usually 10 minutes, to update).';
	$sc .= '</div>';
	...
	$sc .= replace_macros(get_markup_template('field_checkbox.tpl'), array(
		'$field'	=> array('jappixmini-encrypt', t('Encrypt Jabber password with Coleganet password'), $encrypt_checked, t('Recommended'), array(t('No'),t('Yes')), 'onchange="' . $onchange . '"')
	));

	$sc .= replace_macros(get_markup_template('field_password.tpl'), array(
		'$field'	=> array('jappixmini-friendica-password', t('Coleganet password'), '', '', '', $encrypt_disabled . ' onchange="jappixmini_set_password();"')
	));

	$sc .= replace_macros(get_markup_template('field_checkbox.tpl'), array(
		'$field'	=> array('jappixmini-autoapprove', t('Approve subscription requests from Coleganet contacts automatically'), $autoapprove, '', array(t('No'),t('Yes')))
	));

	$sc .= replace_macros(get_markup_template('field_checkbox.tpl'), array(
		'$field'	=> array('jappixmini-autosubscribe', t('Approve subscription requests from Coleganet contacts automatically'), $autosubscribe, '', array(t('No'),t('Yes')))
	));
...
function jappixmini_settings_post(&$a,&$b) {
	...
	if($_POST['jappixmini-submit']) {
		...
		if ($encrypt) {
			...
			if (! $valid) {
				info("Coleganet password not valid.");
				return;
			}
		}
